...
import { NodeEvent,EventTarget } from './EventTarget'
import type { Viewport } from 'src/math/Viewport'
...
export class EventSystem {
    ...
    /** 当前 hover 的元素链（从目标到根） */
    private _hoverEl: EventTarget
    /** 当前按下的元素（用于 click 判定） */
    private _pressedTarget: EventTarget | null = null
    ...
    /** 上次 click 目标 */
    private _lastClickTarget: EventTarget | null = null
    ...
    /** 拖拽源元素 */
    private _dragSource: EventTarget | null = null
    /** 拖拽按下时的屏幕坐标 */
    ...
    private _dragHoverEl: EventTarget | null = null
    ...
    start(inputType: InputType = 'auto'): void {
        ...
        const dom = this.options.domEventTarget
        this._dom = dom
    ...
    private _dispatchPointer(
        ...
        // 转换为世界坐标
        const viewport: Viewport = this.options.viewport
        ...
        // 命中测试
        const target = this.options.pick(worldX, worldY)
        // 创建事件对象
