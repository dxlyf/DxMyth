...
export class Point {
    ...
    public notEquals(other:Point): boolean {
        return !fltEquals(this.x, other.x)
            || !fltEquals(this.y, other.y);
    }
...
export class PolyNode {
    ...
    public polygon:Path = new Array<Point>();
    public index:number;
...
class TEdge {
    public bot:Point;
    public curr:Point; //current (updated for every new scanbeam)
    public top:Point;
    public delta:Point;
...
function PointIsVertex(pt:Point, pp:OutPt):boolean {
    let pp2 = pp;
    do {
        if (pp2.pt.equals(pt)) {
            return true;
...
function PointOnPolygon(pt:Point, pp:OutPt):boolean {
    let pp2 = pp;
    do {
...
function SlopesEqual3P(pt1:Point, pt2:Point, pt3:Point):boolean {
...
function InitEdge(
    e:TEdge, eNext:TEdge, ePrev:TEdge, pt:Point):void {
    e.next = eNext;
    e.prev = ePrev;
    e.curr = new Point(pt.x, pt.y);
    e.outIdx = Unassigned;
}

function InitEdge2(e:TEdge, polyType:PolyType):void {
    if (e.curr.y >= e.next.curr.y) {
        e.bot = new Point(e.curr.x, e.curr.y);
        e.top = new Point(e.next.curr.x, e.next.curr.y);
    } else {
        e.top = new Point(e.curr.x, e.curr.y);
        e.bot = new Point(e.next.curr.x, e.next.curr.y);
    }
    SetDx(e);
    e.polyTyp = polyType;
}
...
function Pt2IsBetweenPt1AndPt3(pt1:Point, pt2:Point, pt3:Point):boolean {
...
function ReverseHorizontal(e:TEdge):void {
    ...
    let top = new Point(e.bot.x, e.top.y);
    let bot = new Point(e.top.x, e.bot.y);
    e.top = top;
    e.bot = bot;
}
...
function GetDx(pt1:Point, pt2:Point):number {
...
function FirstIsBottomPt(this:any,btmPt1:OutPt, btmPt2:OutPt):boolean {
    let p = btmPt1.prev;
    ...
    if (Math.max(dx1p, dx1n) == Math.max(dx2p, dx2n)
        ...
        return (this as any).Area(btmPt1) > 0; //if otherwise identical use orientation
    }
...
function IntersectPoint(edge1:TEdge, edge2:TEdge):Point {
    let b1:number;
    let b2:number;
    let ipx:number;
    let ipy:number;
...
function DupOutPt(outPt:OutPt, insertAfter:boolean):OutPt {
    ...
    result.pt = new Point(outPt.pt.x, outPt.pt.y);
...
function PointInPolygonPath(pt:Point, path:Path):number {
...
//See "The Point in Polygon Problem for Arbitrary Polygons" by Hormann & Agathos
//http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.88.5498&rep=rep1&type=pdf
function PointInPolygonOutPt(pt:Point, op:OutPt):number {
    //returns 0 if false, +1 if true, -1 if pt ON polygon boundary
    let result = 0;
    let startOp = op;
    let ptx = pt.x;
...
function PointInPolygon(pt:Point, shape:OutPt|Path):number {
    if (shape instanceof OutPt) {
...
function DistanceSqrd(pt1:Point, pt2:Point):number {
    let dx = pt1.x - pt2.x;
    let dy = pt1.y - pt2.y;
    return dx*dx + dy*dy;
}

function DistanceFromLineSqrd(pt:Point, ln1:Point, ln2:Point):number {
...
function SlopesNearCollinear(pt1:Point, pt2:Point, pt3:Point, distSqrd:number):boolean {
...
function PointsAreClose(pt1:Point, pt2:Point, distSqrd:number):boolean {
...
function GetUnitNormal(pt1:Point, pt2:Point):Point {
    let dx = pt2.x - pt1.x;
    let dy = pt2.y - pt1.y;
    if (dx == 0 && dy == 0) {
...
class ClipperBase {
    ...
    protected Reset():void {
        ...
        while (lm != null) {
            ...
            if (e != null) {
                e.curr = new Point(e.bot.x, e.bot.y);
                e.outIdx = Unassigned;
            }
            e = lm.rightBound;
            if (e != null) {
...
export class Clipper extends ClipperBase {
    ...
    private AddJoin(Op1:OutPt, Op2:OutPt, OffPt:Point):void {
        let j = new Join(Op1, Op2, OffPt);
        this.m_Joins.push(j);
    }

    private AddGhostJoin(Op:OutPt, OffPt:Point):void {
    ...
    private AddLocalMinPoly(e1:TEdge, e2:TEdge, pt:Point):OutPt {
        let result:OutPt;
        let e:TEdge;
        let prevE:TEdge;
        ...
        if (prevE != null 
            ...
            if (fltEquals(xPrev, xE)
                && e.windDelta != 0
                && prevE.windDelta != 0
                && SlopesEqual4P(new Point(xPrev, pt.y), prevE.top, new Point(xE, pt.y), e.top)) {
    ...
    private AddOutPt(e:TEdge, pt:Point):OutPt {
        if (e.outIdx < 0) {
    ...
    private IntersectEdges(e1:TEdge, e2:TEdge, pt:Point):void {
    ...
    private ProcessHorizontal(horzEdge:TEdge):void {
        ...
        for (;;) {//loop through consec. horizontal edges
            ...
            while(e != null) {
                    ...
                    if (dir == Direction.dLeftToRight) {
                        while (currMax != null && currMax.x < e.curr.x) {
                            if (horzEdge.outIdx >= 0 && !IsOpen) {
                                this.AddOutPt(horzEdge, new Point(currMax.x, horzEdge.bot.y));
                            }
                            currMax = currMax.next;                  
                        }
                    } else {
                ...
                if (dir == Direction.dLeftToRight) {
                    let Pt = new Point(e.curr.x, horzEdge.curr.y);
            ...
            if (IsMaximaEdge) {
                ...
                if (this.StrictlySimple) {
                    ...
                    if (e.outIdx >= 0
                        ...
                        && ePrev.windDelta != 0) {
                        let ip = new Point(e.curr.x, e.curr.y);
    ...
    private JoinHorz(op1:OutPt, op1b:OutPt, op2:OutPt, op2b:OutPt, Pt:Point, DiscardLeft:boolean):boolean {
    ...
    public static CleanPolygon(path:Path, distance:number = 1.415):Path {
        ...
        for (let i = 0; i < cnt; ++i) {
            outPts[i].pt = new Point(path[i].x, path[i].y);
    ...
    private static Minkowski(pattern:Path, path:Path, IsSum:boolean, IsClosed:boolean): Paths {
        ...
        if (IsSum) {
            for (let i = 0; i < pathCnt; i++) {
                let p = new Array<Point>();
                for (let ip of pattern) {
                    p.push(new Point(path[i].x + ip.x, path[i].y + ip.y));
                }
                result.push(p);
            }
        } else {
            for (let i = 0; i < pathCnt; i++) {
                let p = new Array<Point>(); //(polyCnt);
                for (let ip of pattern) {
                    p.push(new Point(path[i].x - ip.x, path[i].y - ip.y));
                }
                result.push(p);
    ...
    public static TranslatePath(path:Path, delta:Point):Path {
        let outPath = new Array<Point>(path.length);
        for (let i = 0; i < path.length; i++) {
            outPath[i] = new Point(path[i].x + delta.x, path[i].y + delta.y);
        }
        return outPath;
    }
...
export class ClipperOffset {
    ...
    private m_normals:Array<Point> = new Array<Point>();
    ...
    private DoOffset(delta:number):void {
        ...
        for (let i = 0; i < this.m_polyNodes.childCount; i++) {
            ...
            this.m_destPoly = new Array<Point>();
            ...
            if (node.endType == EndType.etClosedLine
                ...
                this.m_normals.push(
                    new Point(this.m_normals[len - 2].x, this.m_normals[len - 2].y));
            }
            ...
            if (node.endType == EndType.etClosedPolygon) {
                ...
                for (let j = len - 1; j > 0; j--) {
                    this.m_normals[j] = new Point(-this.m_normals[j - 1].x, -this.m_normals[j - 1].y);
                }
                this.m_normals[0] = new Point(-n.x, -n.y);
                k = 0;
                for (let j = len - 1; j >= 0; j--) {
                    k = this.OffsetPoint(j, k, node.joinType);
                }
                ...
                if (node.endType == EndType.etOpenButt) {
                    ...
                    k = len - 2;
                    this.m_sinA = 0;
                    this.m_normals[j] = new Point(-this.m_normals[j].x, -this.m_normals[j].y);
                ...
                this.m_normals[0] = new Point(-this.m_normals[1].x, -this.m_normals[1].y);
    ...
    private ExecutePaths(solution:Paths, delta:number):void {
        ...
        if (delta > 0) {
            ...
            let outer = new Array<Point>(4);

            outer[0] = new Point(r.left - 10, r.bottom + 10);
            outer[1] = new Point(r.right + 10, r.bottom + 10);
            outer[2] = new Point(r.right + 10, r.top - 10);
            outer[3] = new Point(r.left - 10, r.top - 10);
