// ============================================================
// CatmullRomCurve3 — 3D Catmull-Rom 样条曲线
// 支持 centripetal（默认）/ chordal / catmullrom 三种参数化
// 移植自 three.js extras/curves/CatmullRomCurve3.js
// ============================================================
...
export class CatmullRomCurve3 extends Curve<Vector3> {
    ...
    constructor(points: Vector3[] = [], closed = false, curveType: CatmullRomCurveType = 'centripetal', tension = 0.5) {
        super()
        this.points = points
        this.closed = closed
    ...
    getPoint(t: number, optionalTarget: Vector3 = new Vector3()): Vector3 {
        ...
        let p0: Vector3, p3: Vector3 // 4 个点（p1 & p2 在下方定义）

        if (this.closed || intPoint > 0) {
            p0 = points[(intPoint - 1) % l]
        } else {
            // 外推首点
            Vector3.subtract(tmp, points[0], points[1]).add(points[0])
            p0 = tmp
        }
        ...
        if (this.closed || intPoint + 2 < l) {
            ...
            // 外推末点
            Vector3.subtract(tmp, points[l - 1], points[l - 2]).add(points[l - 1])
            p3 = tmp
        }

        if (this.curveType === 'centripetal' || this.curveType === 'chordal') {
            // 初始化 Centripetal / Chordal Catmull-Rom
            const pow = this.curveType === 'chordal' ? 0.5 : 0.25
            let dt0 = Math.pow(p0.distanceSquaredTo(p1), pow)
            let dt1 = Math.pow(p1.distanceSquaredTo(p2), pow)
            let dt2 = Math.pow(p2.distanceSquaredTo(p3), pow)
    ...
    fromJSON(json: Record<string, any>): this {
        ...
        for (let i = 0, l = json.points.length; i < l; i++) {
            this.points.push(Vector3.fromArray(json.points[i]))
        }
