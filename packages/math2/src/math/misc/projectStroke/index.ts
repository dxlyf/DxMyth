...
import { findIndexRight } from '../../MathUtils';
...
export const projectStrokeOnPoints = (
  ...
  const reduced = points.reduce(
    (reduced, point) => {
      if (!reduced[reduced.length - 1].equals(point)) {
        reduced.push(Vector2.from(point));
      }
      return reduced;
    },
  ...
  if (reduced.length === 1) {
    ...
    const index = findIndexRight(reduced, (point) => !point.equals(start));
    reduced.splice(index + 1);
  }

  reduced.forEach((A, index, points) => {
    let B: Vector2Like, C: Vector2Like;
    if (index === 0) {
      C = points[1];
