...
export const createVector = (from: XY, to: XY): Point =>
  Point.from(to).subtract(from);

/**
 * return the magnitude of a vector
 * @return {number}
 */
export const magnitude = (point: Point) => point.distanceTo(zero);
...
export const getUnitVector = (v: Point): Point =>
  v.equals(zero) ? v : v.divideScalar(magnitude(v));
...
export const getOrthonormalVector = (
  ...
  getUnitVector(new Point(-v.y, v.x).multiplyScalar(counterClockwise ? 1 : -1));
...
export const isBetweenVectors = (t: Point, a: Point, b: Point): boolean => {
  if (t.equals(a) || t.equals(b)) return true;
