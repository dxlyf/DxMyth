...
function orient2dadapt(ax: number, ay: number, bx: number, by: number, cx: number, cy: number, detsum: number): number {
    ...
    let bvirt: number, c: number, ahi: number, alo: number, bhi: number, blo: number, _i: number, _j: number, _0: number, s1: number, s0: number, t1: number, t0: number, u3: number;
...
class Delaunator<T extends ArrayLike<number> = Float64Array> {
    ...
    static from<P>(points: P[], getX?: (point: P) => number, getY?: (point: P) => number): Delaunator<Float64Array> {
        ...
        const gx = getX || defaultGetX;
        const gy = getY || defaultGetY;
    ...
    private _hullPrev: Uint32Array; // edge to prev edge
...
function defaultGetX(p: any): number {
    return p[0];
}
function defaultGetY(p: any): number {
    return p[1];
}
