...
import { MagicString } from "src/utils/MagicString"
...
function isMatrixType(type: string): type is GLSLMatrix {
    return /^mat(2|3|4)(x[234])?$/.test(type)
}

function isSamplerType(type: string): type is GLSLSampler {
    return /^sampler/.test(type)
}
function isScalarType(type: string): type is GLSLScalar {
...
/** GLSL 类型名 → gl 上的 WebGL 枚举常量名 */
const GLSL_TYPE_GL_CONST: Record<string, string> = {
    float: 'FLOAT',
    int: 'INT',
    uint: 'UNSIGNED_INT',
    bool: 'BOOL',
    ...
    ivec3: 'INT_VEC3',
    ivec4: 'INT_VEC4',
    uvec2: 'UNSIGNED_INT_VEC2',
    uvec3: 'UNSIGNED_INT_VEC3',
    ...
    sampler2DMS: 'SAMPLER_2D_MULTISAMPLE',
    sampler2DMSArray: 'SAMPLER_2D_MULTISAMPLE_ARRAY',
    isampler2D: 'INT_SAMPLER_2D',
    isampler3D: 'INT_SAMPLER_3D',
    isamplerCube: 'INT_SAMPLER_CUBE',
    isampler2DArray: 'INT_SAMPLER_2D_ARRAY',
    isampler2DMS: 'INT_SAMPLER_2D_MULTISAMPLE',
    isampler2DMSArray: 'INT_SAMPLER_2D_MULTISAMPLE_ARRAY',
    usampler2D: 'UNSIGNED_INT_SAMPLER_2D',
    usampler3D: 'UNSIGNED_INT_SAMPLER_3D',
    usamplerCube: 'UNSIGNED_INT_SAMPLER_CUBE',
    usampler2DArray: 'UNSIGNED_INT_SAMPLER_2D_ARRAY',
    usampler2DMS: 'UNSIGNED_INT_SAMPLER_2D_MULTISAMPLE',
    usampler2DMSArray: 'UNSIGNED_INT_SAMPLER_2D_MULTISAMPLE_ARRAY',
}
/**
     * 根据 WebGL 类型枚举（如 gl.FLOAT_VEC2）获取对应类型信息
     * 例：gl.FLOAT_VEC2 → { type: 'vec2', itemSize: 2, size: 8, align: 8 }
     *     gl.FLOAT_VEC3 → { type: 'vec3', itemSize: 3, size: 12, align: 16 }
     *     gl.INT_VEC2   → { type: 'ivec2', itemSize: 2, size: 8, align: 8 }
     */
let _typeInfoMap: Map<number, GLTypeInfo> | null =null 
const getTypeInfo=(gl:WebGL2RenderingContext,glType: number): GLTypeInfo | null => {
    if (!_typeInfoMap) {
        _typeInfoMap = new Map<number, GLTypeInfo>()
        for (const [type, constName] of Object.entries(GLSL_TYPE_GL_CONST)) {
            const enumValue = gl[constName as keyof WebGL2RenderingContext]
            if (typeof enumValue !== 'number') continue
            const itemSize = (GLSLTypeSize[type as GLSLPrimitiveType] ?? 4) / 4
            _typeInfoMap.set(enumValue, {
                type,
                itemSize,
                size: GL_STD140_SIZE[type] ?? itemSize * 4,
                align: GL_STD140_ALIGNMENT[type] ?? 4,
            })
        }
    }
    return _typeInfoMap.get(glType) ?? null
}
export class GLProgram {
    ...
    private _typeInfoMap: Map<number, GLTypeInfo> | null = null
