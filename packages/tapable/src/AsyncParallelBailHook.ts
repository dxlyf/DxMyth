...
import { HookAsyncCallback } from "./types";
 ...
 class AsyncParallelBailHook<T extends any[],Result> extends Hook<T,Result|undefined> {
	...
	callAsync(..._args: [...T, HookAsyncCallback<Result|undefined>]): void {
		 let cb=_args.pop()!
		 this.callTapsParallel('Bail',this.maxParallel,_args as any,cb)
	}
	promise(..._args: T): Promise<Result|undefined> {
		return new Promise((resolve,reject)=>{
			this.callTapsParallel('Bail',this.maxParallel,_args,(err,res:Result|undefined)=>{
				if(err){
					reject(err)
					return
				}
