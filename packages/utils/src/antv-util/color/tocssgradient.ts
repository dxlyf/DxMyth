...
function isGradientColor(val:string) {
  return /^[r,R,L,l]{1}[\s]*\(/.test(val);
}

export function toCSSGradient(gradientColor:any) {
  if (isGradientColor(gradientColor)) {
    let cssColor='';
    let steps;
    if (gradientColor[0] === 'l') {
      // 线性渐变
      const arr = regexLG.exec(gradientColor) as any;
      ...
      const arr = regexRG.exec(gradientColor) as any;
      steps = arr[4];
    }
