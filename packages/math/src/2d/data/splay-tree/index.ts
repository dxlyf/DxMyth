...
export type Visitor<Key, Value> = (node:Node<Key, Value>) => void|boolean;
 ...
 */
function splay (i:Key, t:Node<Key, Value>, comparator:Comparator<Key>) : Node<Key, Value> {
...
export default class Tree<Key=number, Value=any> {
  ...
  private _remove (
    ...
    comparator:Comparator<Key>) : Node<Key, Value>|null {
    let x;
  ...
  public forEach (visitor:Visitor<Key, Value>, ctx?:any) : Tree<Key, Value> {
    ...
    while (!done) {
      if (current !==  null) {
        ...
        if (Q.length !== 0) {
          current = Q.pop() as Node<Key, Value>;
