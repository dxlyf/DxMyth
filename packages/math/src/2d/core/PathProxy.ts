...
import {vec2,Vec2Like} from 'gl-matrix/vec2';
import BoundingRect from './BoundingRect';

import { fromLine, fromCubic, fromQuadratic, fromArc } from './bbox';
import { cubicLength, cubicSubdivide, quadraticLength, quadraticSubdivide } from 'src/2d/curve/curve';
// 判断是否是浏览器环境
const isBrowser = () => 
...
const tmpOutX: Vec2Like = vec2.create();
const tmpOutY: Vec2Like =vec2.create();

const min: Vec2Like = vec2.create();
const max: Vec2Like= vec2.create();
const min2: Vec2Like = vec2.create();
const max2: Vec2Like= vec2.create();
...
export default class PathProxy {
    ...
    private _pathSegLen!: number[]|null
    ...
    arc(cx: number, cy: number, r: number, startAngle: number, endAngle: number, anticlockwise: boolean=false) {
        this._drawPendingPt();
    ...
    rebuildPath(ctx: PathRebuilder, percent: number) {
        ...
        let pendingPtDist = 0;
        let pendingPtX: number=0;
        let pendingPtY: number=0;
        ...
        if (drawPart) {
            ...
            pathSegLen = this._pathSegLen as number[];
            ...
            switch (cmd) {
                ...
                case CMD.C: {
                    ...
                    if (drawPart) {
                        ...
                        if (accumLength + l > displayedLength) {
                            const t = (displayedLength - accumLength) / l;
                            cubicSubdivide(xi, x1, x2, x3, t, tmpOutX as any);
                            cubicSubdivide(yi, y1, y2, y3, t, tmpOutY as any);
                ...
                case CMD.Q: {
                    ...
                    if (drawPart) {
                        ...
                        if (accumLength + l > displayedLength) {
                            const t = (displayedLength - accumLength) / l;
                            quadraticSubdivide(xi, x1, x2, t, tmpOutX as any);
                            quadraticSubdivide(yi, y1, y2, t, tmpOutY as any);
