...
export function clone<T extends any>(source: T): T {
    ...
    if (typeStr === '[object Array]') {
        ...
        if (!isPrimitive(source)) {
            ...
            if (Ctor.from) {
                result = Ctor.from(source as any);
            }
            else {
                result = new Ctor((source as any).length);
                for (let i = 0, len = (source as any).length; i < len; i++) {
                    result[i] = (source as any)[i];
                }
            }
        }
    }
...
export function mixin(target: any, source:any, override?: boolean) {
...
export function slice<T>(arr: ArrayLike<T>, ...args: SliceParams): T[] {
    return nativeSlice.apply(arr, args as SliceParams);
}
