 ...
 */
this.PIXI = this.PIXI || {};
var html_source_js = (function (exports) {
    'use strict';
    ...
    class ElementImageSource extends PIXI.TextureSource {
      /**
    ...
    ElementImageSource.extension = {
      type: PIXI.ExtensionType.TextureSource,
      priority: -10
    };
    ...
    const glUploadHTMLResource = {
      extension: {
        type: PIXI.ExtensionType.TextureUploaderWebGL,
        name: "html"
      },
      id: "html",
      upload(source, glTexture, gl, _webGLVersion, targetOverride) {
        ...
        const target = targetOverride != null ? targetOverride : glTexture.target;
        const textureWidth = source.pixelWidth;
        const textureHeight = source.pixelHeight;
        if (!source.isReady) {
          ensureAllocated(gl, glTexture, target, textureWidth, textureHeight);
          (_a = source.requestPaint) == null ? void 0 : _a.call(source);
          return;
        }
        if (upload.length === 3) {
    ...
    const gpuUploadHTMLResource = {
      extension: {
        type: PIXI.ExtensionType.TextureUploaderWebGPU,
        name: "html"
      },
      type: "html",
    ...
    const _HTMLSource = class _HTMLSource extends PIXI.TextureSource {
      /**
      ...
      constructor(options) {
        var _a;
        options = __spreadValues(__spreadValues({}, _HTMLSource.defaultOptions), options);
        if (!options.resource) {
        ...
        /** The upload method for this texture. */
        this.uploadMethodId = "html";
        const canvas = (_a = options.canvas) != null ? _a : this._inferCanvas(options.resource);
        if (!canvas) {
          throw new Error(
            // eslint-disable-next-line max-len
      ...
      requestPaint() {
        var _a;
        if (!((_a = this.canvas) == null ? void 0 : _a.requestPaint)) {
          return false;
        }
      ...
      _onPaint(event) {
        ...
        if ((changedElements == null ? void 0 : changedElements.length) && !changedElements.includes(this.resource)) {
          return;
        }
        this._isReady = true;
        this.update();
    ...
    exports.ElementImageSource = ElementImageSource;
    exports.HTMLSource = HTMLSource;
    exports.glUploadHTMLResource = glUploadHTMLResource;
    exports.gpuUploadHTMLResource = gpuUploadHTMLResource;

    return exports;

})({});
Object.assign(this.PIXI, html_source_js);
