import type { INodeEventTarget, INodeEvent, EventCallbackOrObject, AddEventListenerOptions, EventListenerOptions } from '../types/EventTarget';

/** 节点事件实现，包含事件冒泡/捕获的生命周期控制 */
class NodeEvent<T = string, D = any> implements INodeEvent<T, D> {
    /**
     * 创建事件实例的工厂方法
     * @param type 事件类型
     * @param data 事件数据
     ...
     * 用于事件捕获和冒泡阶段的遍历
     */
    composedPath(): INodeEventTarget[] {
        let current = this.target as INodeEventTarget
        let path: INodeEventTarget[] = []
        while (current) {
            path.push(current)
...
class EventTarget<EventListeners extends Record<string, any> = Record<string, any>> implements INodeEventTarget<EventListeners> {
    /** NodeEvent 类引用，方便外部创建事件 */
    static readonly NodeEvent = NodeEvent
    /** 事件监听器存储，key 格式为 "type"（冒泡）或 "type_capture"（捕获） */
    listeners: Map<string, EventCallbackOrObject<any, any>[]> = new Map();
    /** 父级事件目标，用于事件冒泡 */
    declare parent: INodeEventTarget<EventListeners> | null;
    ...
    addEventListener<K extends Extract<keyof EventListeners, string>>(
        type: K,
        handler: EventCallbackOrObject<K, EventListeners[K]>,
    ...
    getEventListeners<K extends Extract<keyof EventListeners, string>>(type: K): EventCallbackOrObject<K, EventListeners[K]>[] {
        ...
        const capture = (this.listeners.get(captureKey) || []) as EventCallbackOrObject<K, EventListeners[K]>[];
        const bubble = (this.listeners.get(bubbleKey) || []) as EventCallbackOrObject<K, EventListeners[K]>[];
    ...
    private _resolveKey(type: string, options?: AddEventListenerOptions): string {
        let capture = false;
    ...
    private _invokeHandler(handler: EventCallbackOrObject<any, any>, event: INodeEvent<any, any>): void {
