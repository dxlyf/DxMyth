...
export type WritableDraft<T> = T extends any[] ? number extends T["length"] ? IsPlainArray<T> extends true ? Draft<T[number]>[] : WritableNonArrayDraft<T> : WritableNonArrayDraft<T> : WritableNonArrayDraft<T>;
/**
 ...
 */
type IsPlainArray<T extends readonly any[]> = T extends readonly (infer U)[] ? U[] extends T ? true : false : false;
