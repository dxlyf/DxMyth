...
function updateOptions(
  ...
  if (multiple) {
    let selectedValues = (propValue as Array<string>);
    let selectedValue = {};
...
export function initWrapperState(element: Element, props: any) {
  const node = (element as SelectWithWrapperState);
...
export function postUpdateWrapper(element: Element, props: any) {
  const node = (element as SelectWithWrapperState);
  const wasMultiple = node._wrapperState.wasMultiple !== !!props.multiple;
