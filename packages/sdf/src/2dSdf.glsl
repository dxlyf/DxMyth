...
float sdPolygon2(vec2 p,float radius,int sides,float rotate){
    float PI=3.1415926;
    float p2=PI*2.;
...
float sdPolygon(vec2 p,float radius,int sides,float rotate){
    float PI=3.1415926;
    float p2=PI*2.;
    float rad=p2/float(sides);
...
类似地，形状可以做成环形（像一个环或洋葱的层），但取它们的绝对值，
然后从它们的场中减去一个常数。
因此，对于任何由d(x,y) = sdf(x,y)定义的形状，计算d(x,y) = |sdf(x,y)| - r：
*/
// 使形状变圆润
...
float map(vec2 uv){
    float d=min(sdCircle(uv-vec2(-0.4,0),0.2),sdBox(uv-vec2(0.3,0),vec2(0.2)));
    d=min(d,abs(sdBox(uv-vec2(0.3,0.5),vec2(0.2)))-0.03);
    d=min(d,sdPolygon2(uv-vec2(-0.4,0.5),0.2,6,0.));
    d=min(d,abs(sdPolygon2(uv-vec2(-0.4,1.0),0.2,6,0.))-0.02);
    return d;
}
void mainImage(out vec4 fragColor, in vec2 fragCoord) {

	vec2 uv =2.*( fragCoord.xy*2. - iResolution.xy )/min(iResolution.x,iResolution.y);
    vec3 color=vec3(1,0,0)*step(map(uv),0.);
