...
/**************2d*********/
float sdLine(vec2 p,vec2 dir,float w){
   vec2 proj=dir*dot(p,dir);
   return length(p-proj)-w*.5;
}
float sdSegment(vec2 p,vec2 a,vec2 b,float w){
   vec2 detal=b-a;
   float t=clamp(dot(p-a,detal)/dot2(detal),0.,1.);
   return length(p-mix(a,b,t))-w*0.5;
}
