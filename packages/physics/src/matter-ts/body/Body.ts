...
export interface IBody {
  ...
  constraintImpulse: IVector & { angle: number }
...
export default class Body {
  protected static _timeCorrection = true
  protected static _inertiaScale = 4
  protected static _nextCollidingGroupId = 1
  protected static _nextNonCollidingGroupId = -1
  protected static _nextCategory = 0x0001
   ...
   * @param options
   */
  protected static _initProperties(
    body: InitBody,
    options: DeepPartial<IBody> = {}
  ): void {
    // init required properties (order is important)
    Body.set(body as IBody, {
      ...
      parent: body.parent || (body as IBody),
    })
    ...
    Body.set(body as IBody, {
      axes: (options.axes as IAxes | undefined) || body.axes!,
   ...
   */
  public static nextGroup(isNonColliding: boolean = false): number {
    if (isNonColliding) {
   ...
   */
  public static nextCategory(): number {
  ...
  public static set<K extends keyof IBody>(
    body: IBody,
    settings: K | 'centre' | Partial<IBody>,
    value?: IBody[K]
  ): void {
    if (typeof settings === 'string') {
      if (!value) {
        Common.warn('Body.set() need 3 arguments')
        return
      }
    ...
    for (const property in settings) {
      ...
      const v = settings[property as keyof IBody]
      Body._setByKey(body, property as keyof IBody, v)
    }
  }

  protected static _setByKey<K extends keyof IBody>(
    body: IBody,
    key: K | 'centre',
    value: IBody[K]
  ) {
    switch (key) {
      case 'isStatic':
        Body.setStatic(body, value as boolean)
        break
      case 'isSleeping':
        Sleeping.set(body, value as boolean)
        break
      case 'mass':
        Body.setMass(body, value as number)
        break
      case 'density':
        Body.setDensity(body, value as number)
        break
      case 'inertia':
      ...
      case 'position':
        ...
        break
      case 'angle':
        Body.setAngle(body, value as number)
        break
      case 'velocity':
        Body.setVelocity(body, value as IVector)
        break
      case 'angularVelocity':
        Body.setAngularVelocity(body, value as number)
        break
      case 'speed':
        Body.setSpeed(body, value as number)
        break
      case 'angularSpeed':
        Body.setAngularSpeed(body, value as number)
        break
      case 'parts':
        Body.setParts(body, value as IBody[])
        break
      case 'centre':
        Body.setCentre(body, value as IVector)
        break
      default:
        body[key] = value
    }
   ...
   * @param isStatic
   */
  public static setStatic(body: IBody, isStatic: boolean): void {
   ...
   * @param mass
   */
  public static setMass(body: IBody, mass: number): void {
   ...
   * @param inertia
   */
  public static setInertia(body: IBody, inertia: number): void {
   ...
   * @param velocity
   */
  public static setVelocity(body: IBody, velocity: IVector): void {
   ...
   * @return speed
   */
  public static getSpeed(body: IBody): number {
   ...
   * @param speed
   */
  public static setSpeed(body: IBody, speed: number): void {
    Body.setVelocity(
      body,
      Vector.mult(Vector.normalise(Body.getVelocity(body)), speed)
    )
  }
   ...
   * @param velocity
   */
  public static setAngularVelocity(body: IBody, velocity: number): void {
    const timeScale = body.deltaTime / Body._baseDelta
   ...
   */
  public static getAngularVelocity(body: IBody): number {
   ...
   */
  public static getAngularSpeed(body: IBody): number {
   ...
   * @param speed
   */
  public static setAngularSpeed(body: IBody, speed: number): void {
    Body.setAngularVelocity(
      body,
      Common.sign(Body.getAngularVelocity(body)) * speed
    )
  }
   ...
   * @param deltaTime
   */
  public static update(body: IBody, deltaTime: number = 1000 / 60): void {
   ...
   * @param body
   */
  public static updateVelocities(body: IBody): void {
    const timeScale = Body._baseDelta / body.deltaTime
  ...
  private static _totalProperties(
    body: IBody
  ): Pick<IBody, 'mass' | 'area' | 'inertia'> & { centre: IVector } {
    // from equations at:
