...
class ImageBitmapLoader extends Loader<ImageBitmap> {
   declare readonly isImageBitmapLoader: boolean;
	declare _abortController: AbortController;
    /**
     * @default { premultiplyAlpha: 'none' }
     */
   declare options: ImageBitmapOptions;
	/**
	 ...
	 */
	load( url:string, onLoad:OnLoadCallback<ImageBitmap>, onProgress?:OnProgressCallback, onError?:OnErrorCallback) {
		...
		const cached = Cache.get( `image-bitmap:${url}` ) as (ImageBitmap|Promise<ImageBitmap>);
		...
		if ( cached !== undefined ) {
			...
			if ( (cached as Promise<ImageBitmap>).then ) {

				(cached as Promise<ImageBitmap>).then( imageBitmap => {
			...
			setTimeout( function () {
				...
				if ( onLoad ) onLoad( cached as ImageBitmap );
