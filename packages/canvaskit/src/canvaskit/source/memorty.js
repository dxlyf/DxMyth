 ...
 * the WASM heap.
 * 
方法	作用	内存管理	路径数据	性能特点
delete()	立即释放路径内存	完全释放	完全清除	最彻底，但需手动调用
deleteLater()	延迟释放路径内存	计划释放	完全清除	避免渲染中断，自动管理
reset()	清空路径数据	保留内存分配	移除所有命令	快速清空，内存结构保留
rewind()	清空路径数据	保留内存分配	移除所有命令	类似reset()，可能更高效
