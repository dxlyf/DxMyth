...
function colorToString(skcolor:CanvasKit.Color) {
  // https://html.spec.whatwg.org/multipage/canvas.html#serialisation-of-a-color
  var components = CK.getColorComponents(skcolor);
  var r:any = components[0];
  var g:any = components[1];
  var b:any = components[2];
  var a:any = components[3];
  if (a === 1.0) {
    // hex
...
function parseColor(colorStr:string) {
  return CK.parseColorString(colorStr, colorMap);
}
