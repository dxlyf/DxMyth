...
function getTypeface(fontstr:string) {
  var descriptors = parseFontString(fontstr) as any;
...
function addToFontCache(typeface:any, descriptors:any) {
...
function getFromFontCache(descriptors:any) {
  var key = (descriptors['style']   || 'normal') + '|' +
            (descriptors['variant'] || 'normal') + '|' +
            (descriptors['weight']  || 'normal');
  var fam = descriptors['family'];
