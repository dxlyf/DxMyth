...
import { TOriginX, TOriginY, TRadian } from './typedefs';
import { saveObjectTransform } from './util/misc/objectTransforms';
import { Canvas } from './canvas/Canvas';
import { IText } from './shapes/IText/IText';
import { StaticCanvas } from './canvas/StaticCanvas';
import { LayoutBeforeEvent, LayoutAfterEvent } from './LayoutManager/types';
import { MODIFIED, MODIFY_PATH, MODIFY_POLY, MOVING, RESIZING, ROTATING, SCALING, SKEWING } from './constants';
...
export type TransformAction<T extends Transform = Transform, R = void> = (eventData: TPointerEvent, transform: T, x: number, y: number) => R;
/**
...
export interface BasicTransformEvent<E extends Event = TPointerEvent> extends TEvent<E> {
    transform: Transform;
    pointer: Point;
}
...
export interface ObjectEvents extends ObjectPointerEvents, DnDEvents, MiscEvents, ObjectModificationEvents {
